...
()=>{shazamme.unsub(m);shazamme.unsub(p)})});if(this._pages[a]?.find(f=>f.parentQuestionID))k.find("input, select").on("change",function(){e._recordAnswers();e._showQuestions(a,!1)});k.find("[data-rel=screening-apply]").click(function(){if(z())shazamme.pub(v.submit,{plugin:e,w:q});else{let f=h?.warningScreeningQuestions||"Please answer all questions";u?.alertDialog({title:h?.warningScreeningQuestionsTitle||"Error",message:f})?.appendTo(k)||alert(f)}});c&&(k.get(0).scrollIntoView(),window.scrollBy({top:-200,
...
c.push(`<div class="sq-pages-index">${b.join("")}</div>`)}c.push("</div>");return c.join("")};this._recordAnswers=()=>{k.find("input, select").each((a,c)=>{a=$(c);switch(a.attr("data-qtype")){case "text":a.val()?e._answers[a.attr("data-qid")]={screeningAnswerID:e._answers[a.attr("data-qid")]?.screeningAnswerID,screeningQuestionID:a.attr("data-qid"),answerText:a.val(),screeningTemplateID:e._screeningTemplateID}:delete e._answers[a.attr("data-qid")];break;case "number":a.val()?e._answers[a.attr("data-qid")]=
{screeningAnswerID:e._answers[a.attr("data-qid")]?.screeningAnswerID,screeningQuestionID:a.attr("data-qid"),answerNum:parseInt(a.val()),screeningTemplateID:e._screeningTemplateID}:delete e._answers[a.attr("data-qid")];break;case "date":a.val()?e._answers[a.attr("data-qid")]={screeningAnswerID:e._answers[a.attr("data-qid")]?.screeningAnswerID,screeningQuestionID:a.attr("data-qid"),answerDate:a.val(),screeningTemplateID:e._screeningTemplateID}:delete e._answers[a.attr("data-qid")];break;case "bool":e._answers[a.attr("data-qid")]=
{screeningAnswerID:e._answers[a.attr("data-qid")]?.screeningAnswerID,screeningQuestionID:a.attr("data-qid"),answerBoolean:a.is(":checked")?1:0,screeningTemplateID:e._screeningTemplateID};break;case "list":a.val()?e._answers[a.attr("data-qid")]={screeningAnswerID:e._answers[a.attr("data-qid")]?.screeningAnswerID,screeningQuestionID:a.attr("data-qid"),answerUUID:a.val(),screeningTemplateID:e._screeningTemplateID}:delete e._answers[a.attr("data-qid")];break;case "check-list":c=a.attr("data-qid");let b=
e._answers[c]?.answerUUID||[];a.is(":checked")?0>b.indexOf(a.attr("data-value"))&&b.push(a.attr("data-value")):0<=b.indexOf(a.attr("data-value"))&&b.splice(b.indexOf(a.attr("data-value")),1);0===b.length?delete e._answers[a.attr("data-qid")]:e._answers[c]={screeningAnswerID:e._answers[c]?.screeningAnswerID,screeningQuestionID:a.attr("data-qid"),answerUUID:b,screeningTemplateID:e._screeningTemplateID};break;case "radio":a.is(":checked")&&(e._answers[a.attr("data-qid")]={screeningAnswerID:e._answers[a.attr("data-qid")]?.screeningAnswerID,
screeningQuestionID:a.attr("data-qid"),answerUUID:a.attr("value"),screeningTemplateID:e._screeningTemplateID})}})};this._restoreAnswers=a=>{for(let c in e._answers){let b=e._answers[c];c=c.split(":")[0];0>a.indexOf(c)||(0===k.find(`input[data-qid=${c}]:visible, select[data-qid=${c}]:visible`).length?delete e._answers[c]:b.answerText||b.answerNum?k.find(`input[data-qid=${c}]`).val(b.answerText||b.answerNum):b.answerDate?k.find(`input[data-qid=${c}]`).val(b.answerDate.substr(0,10)):b.answerBoolean&&
!b.answerUUID?k.find(`input[data-qid=${c}]`).attr("checked",!0):b.answerUUID?("string"===typeof b.answerUUID&&(b.answerUUID=b.answerUUID.split(",")),b.answerUUID.forEach(d=>{k.find(`input[data-qid=${c}][data-value=${d.trim()}]`).attr("checked",!0);k.find(`input[data-qid=${c}][value=${d.trim()}]`).attr("checked",!0);k.find(`select[data-qid=${c}]`).val(d.trim())})):b.answerFile&&k.find(`input[data-qid=${c}]`).val(this._fileBlob(b.answerFile)))}};this._readFile=a=>new Promise(c=>{let b=new FileReader;
b.addEventListener("load",()=>{c(btoa(b.result))},!1);b.readAsBinaryString(a)});this._fileBlob=(a,c="",b=512)=>{a=atob(a);const d=[];for(let g=0;g<a.length;g+=b){var f=a.slice(g,g+b);const l=Array(f.length);for(let m=0;m<f.length;m++)l[m]=f.charCodeAt(m);f=new Uint8Array(l);d.push(f)}return new Blob(d,{type:c})};this._showEditor=()=>{const a=$('\n                    <div class="dialog dialog-knockout editor-only" data-rel="dialog" data-dialog="knockout">\n                        <div class="content" data-rel="dialog-content"></div>\n\n                        <div class="buttons">\n                            <button class="submit" data-rel="button-submit"><span class="text">OK</span></button>\n                            <button class="dismiss" data-rel="button-dismiss"><span class="text">Cancel</span></button>\n                        </div>\n                    </div>\n                ').appendTo(k),
...
t&&this._showEditor();return Promise.resolve({nextPage:C,prevPage:D,answers:B,loadAnswers:E,knockout:F,container:k,validate:z,message:v,version:"1.0.0"})})}}})();